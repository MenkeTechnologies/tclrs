...
fn constructs_that_cannot_be_resolved_while_compiling_are_refused() {
    for (src, expected) in [
        ...
        //
        // The definition alone used to be enough to fail. It is a *deferred*
        // refusal now — tclsh compiles a procedure's body when the procedure is
        // first called, so `proc p {} {<refused>}` never called runs to
        // completion there — so the call is what reaches it, and the entry says
        // so by calling.
        (
