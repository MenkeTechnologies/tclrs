...
/// What Tk makes of [`script`]'s file name: the tail of `argv0` with its first
/// character titlecased, which is the main window's class.
fn titlecased_script_name(name: &str) -> String {
...
fn package_require_tk_loads_and_initialises_the_toolkit() {
    ...
    // The main window's class is `Tcl_UtfToTitle` of the application name, and
    // the application name is the tail of `argv0`
    // (`tk9.0.4/macosx/tkMacOSXInit.c:783-799`, then
    // `tk9.0.4/generic/tkWindow.c:3363-3375`) — so it is this script's own file
    // name with its first letter titlecased, not the constant `Tk`.
    //
    // It *was* `Tk` here, and only because `argv0` could not be read: before
    // the variable bridge landed, `Tcl_GetVar2` answered NULL and
    // `TkpGetAppName` fell back to its literal `"tk"`. Measured against the
    // reference interpreter on the same shape of input, which agrees:
    //
    //   $ printf 'package require Tk\nputs [winfo class .]\nexit\n' > /tmp/t6b.tcl
    //   $ tclsh9.0 /tmp/t6b.tcl
    //   T6b.tcl
    //
    // Still a canary for the same thing the constant was: the main window
    // exists and Tk answers `winfo` about it through a command this frontend
    // resolved at run time.
    assert_eq!(
        field(&ran.stdout, "CLASS").map(str::to_string),
        Some(titlecased_script_name("require")),
        "the main window's class is not the titlecased application name"
    );
