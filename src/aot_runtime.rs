...
pub unsafe extern "C" fn fusevm_aot_register_builtins(vm: *mut VM) {
    ...
    // `install_hooks` supplies the buffered stdout sink along with everything
    // else, so a linked binary writes exactly what the interpreter would.
    let hooks = crate::runtime::install_hooks(vm);
    ...
    *PARKED.lock().expect("parked lock") = Some(hooks);
}

/// The hooks of the run in progress, readable after fusevm's AOT entry has
/// dropped the VM.
static PARKED: Mutex<Option<crate::runtime::Hooks>> = Mutex::new(None);
...
pub extern "C" fn tclrs_aot_report_error() -> i32 {
    ...
    match parked.and_then(|h| h.take_error()) {
        Some(msg) => {
            eprintln!("tclrs: {msg}");
            1
        }
