...
pub unsafe fn append_bytes_to_obj(obj: *mut TclObj, bytes: &[u8]) {
    let mut all = obj_bytes(obj).to_vec();
    free_internal_rep(obj);
...
pub unsafe fn result_bytes(interp: *mut c_void) -> Vec<u8> {
    ...
    obj_bytes(h.result).to_vec()
}

/// Install `obj` as the interpreter result, releasing the previous one.
///
/// # Safety
/// `interp` is a `Tcl_Interp *` this crate handed to Tk and `obj` is live.
unsafe fn store_result(interp: *mut c_void, obj: *mut TclObj) {
    let h = &mut *(*(interp as *mut HostInterp)).host;
    (*obj).ref_count += 1;
...
/// The body of `TclFreeObj` without the trace line, so that a value this side
/// released — through [`release_obj`] — is not logged as a call Tk made.
///
/// # Safety
/// `obj` is null or a live `Tcl_Obj` whose last reference has gone.
unsafe fn free_obj(obj: *mut TclObj) {
    if obj.is_null() {
        return;
    }
...
unsafe extern "C" fn dict_obj_put(
    ...
    if let Some(slot) = d.pairs.iter_mut().find(|(k, _)| obj_bytes(*k) == want) {
        release_obj(std::mem::replace(&mut slot.1, value));
    } else {
        (*key).ref_count += 1;
