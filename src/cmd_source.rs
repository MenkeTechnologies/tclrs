...
/// Extension opcode ids owned by this module, sharing the `NS_BASE` block with
/// [`crate::cmd_namespace::ext`] — see the note there.
pub mod ext {
    use crate::compiler::ext::NS_BASE;

    /// `[path]` → the value of the file's last command.
    pub const SOURCE: u16 = NS_BASE + 2;
    /// `[basename, version, patch, initScript, enVarName, varName]` → `""`,
    /// having sourced the script it found.
    pub const FIND_LIBRARY: u16 = NS_BASE + 3;
}
