...
**29335 of 48201 attempted cases pass — 60.9%.** Over every case the suite contains, including the ones that cannot be run here, that is 29335 of 69424 — 42.3%.
...
| Skipped — cannot be run | 21223 | 30.6% |
| Attempted | 48201 | 69.4% |
| ⤷ passed | 29335 | 60.9% of attempted |
| ⤷ failed | 18866 | 39.1% of attempted |

Of the 18866 failures, 15912 are a feature tclrs documents as not built yet rather than a wrong answer. Counting those as skips instead would give 29335 of 32289 — 90.9% — and that looser number is stated here only so the choice of rule is visible. The headline above uses the strict rule.
...
| tclrs has no such command | 2809 |
...
| `timerate` | 15 |
| `tailcall` | 14 |
| `tcl::unsupported::representation` | 14 |
| `unload` | 14 |
| `zlib` | 14 |
...
| `auto_qualify` | 8 |
| `tcl::tm::path` | 8 |
| *49 further commands* | 116 |

## Why cases failed

| Cause | Cases | Share of failures | For example |
| --- | ---: | ---: | --- |
| tclrs raised an error, tclsh did not | 11084 | 58.8% | `append.test` append-7.1, `append.test` append-7.2, `append.test` append-7.3 |
| both raised an error, messages differ | 6647 | 35.2% | `append.test` append-3.1, `append.test` append-3.2, `append.test` append-6.1 |
| results differ | 976 | 5.2% | `append.test` append-3.4, `append.test` append-3.5, `append.test` append-3.6 |
| tclsh raised an error, tclrs did not | 143 | 0.8% | `appendComp.test` appendComp-10.4, `clock-ivm.test` clock-11.1.vm:0, `clock-ivm.test` clock-11.2.vm:0 |
| tclrs was killed or crashed | 16 | 0.1% | `clock-ivm.test` clock-6.0.vm:0, `clock.test` clock-6.0.vm:1, `dict.test` dict-14.16 |
...
| command name must be a literal in this phase | 1154 |
| identical text apart from tclrs's trailing (line N) | 296 |
| can't read "…": no such variable | 273 |
| "…" outside of a procedure is not supported | 242 |
| wrong # args: should be "…"; the options variable is not supported | 226 |
...
| expression must be a literal in this phase | 151 |
...
| script body must be a literal in this phase | 51 |
| unable to convert input string: ambiguous day | 48 |
| variable name must be a literal in this phase | 39 |
...
| "…" is not supported yet: this frontend resolves namespaces while compiling, so the name has to be written out | 23 |
| array startsearch is not supported yet | 23 |
...
| `append.test` | 52 | 8 | 44 | 30 | 14 | 68.2% |
| `appendComp.test` | 48 | 12 | 36 | 29 | 7 | 80.6% |
| `apply.test` | 42 | 6 | 36 | 5 | 31 | 13.9% |
...
| `expr.test` | 2168 | 1094 | 1074 | 868 | 206 | 80.8% |
...
| `for.test` | 88 | 26 | 62 | 18 | 44 | 29.0% |
| `foreach.test` | 43 | 1 | 42 | 34 | 8 | 81.0% |
| `format.test` | 269 | 1 | 268 | 262 | 6 | 97.8% |
...
| `incr.test` | 69 | 2 | 67 | 24 | 43 | 35.8% |
| `indexObj.test` | 65 | 65 | 0 | 0 | 0 | — |
| `info.test` | 287 | 136 | 151 | 61 | 90 | 40.4% |
| `init.test` | 10 | 10 | 0 | 0 | 0 | — |
| `interp.test` | 355 | 299 | 56 | 0 | 56 | 0.0% |
| `io.test` | 884 | 481 | 403 | 377 | 26 | 93.5% |
...
| `lseq.test` | 136 | 23 | 113 | 81 | 32 | 71.7% |
...
| `namespace-old.test` | 126 | 32 | 94 | 62 | 32 | 66.0% |
| `namespace.test` | 314 | 70 | 244 | 114 | 130 | 46.7% |
| `notify.test` | 23 | 23 | 0 | 0 | 0 | — |
| `nre.test` | 28 | 23 | 5 | 0 | 5 | 0.0% |
| `obj.test` | 84 | 76 | 8 | 7 | 1 | 87.5% |
| `oo.test` | 388 | 262 | 126 | 0 | 126 | 0.0% |
| `ooNext2.test` | 62 | 32 | 30 | 0 | 30 | 0.0% |
| `ooProp.test` | 55 | 38 | 17 | 0 | 17 | 0.0% |
| `ooUtil.test` | 33 | 18 | 15 | 0 | 15 | 0.0% |
...
| `tailcall.test` | 37 | 27 | 10 | 0 | 10 | 0.0% |
