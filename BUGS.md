...
- **`expr`.** The whole operator set of `expr(n)` with `expr(n)` precedence,
  ...
  spelling as well as its value, so `expr {1.0 eq 1}` is 0 like tclsh's. Almost
  nothing converts an `expr` *result* — it stays the integer, double or boolean
  the VM computed, and Tcl's string form is applied where a string is asked for,
  which is what leaves an arithmetic loop lowerable by the JIT and the
  ahead-of-time compiler. The exceptions are the two the VM's value cannot
  spell: a result that can be a NaN, which `expr(n)` reports rather than answers,
  and a bare operand, which answers with the number it spells (`expr {007}` is
  7). `Compiler::normalizes` decides which, and answers no for every counting
  and accumulating loop whose second operand is written out.
...
whatever diverges. One run of 400 programs (`-n 400 -s 1`, 67 s) puts 146 in
parity, 167 in divergence, 63 in skip, 22 in the allowlist and 2 outside
comparison because tclsh did not terminate. **157 of the 167 are the one class
below that is not a defect** — a script's shape refused while compiling, which
lands as a message on a channel tclsh never reached — so 10 are behavior.

A later, wider run (`-n 2000 -s 1 -d 4`, 310 s) puts 658 in parity, 949 in
divergence, 267 in skip, 119 in the allowlist and 7 outside comparison. 903 of
the 949 are again the compile-time class; 46 are behavior.

A third seed (`-n 2000 -s 1001 -d 4`, 331 s) puts 643 in parity, 949 in
divergence, 300 in skip, 99 in the allowlist and 9 outside comparison. The same
command against the two branches this tree merges puts 419 and 621 in parity and
1253 and 982 in divergence, so neither line of work alone reaches what the two
together do.

Mutation mode reaches the same buckets from the other direction: `-M -n 500 -s 21
-m` recombines the committed corpus and puts 120 in parity, 300 in divergence, 18
in skip and 62 in the allowlist, with **no case in `CRITICAL` and none in
...
What is left of the seven the 2000-program run above reached; the rest are in
the fixed lists below. Each is pinned in `tests/parity_fuzz_findings.rs` against
a live tclsh, and each is reachable only because the generator now builds
`format`'s specifier matrix, draws shift counts with a sign, and carries `nan` /
`inf` in its value pools.
